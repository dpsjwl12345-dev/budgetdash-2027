...
export default function Home() {
  ...
  return (
    <Layout showToast={showToast}>
      ...
      {showStaffModal && <div className="modal-backdrop" onMouseDown={() => setShowStaffModal(false)}><div className="modal-card" ref={staffModalRef} role="dialog" aria-modal="true" aria-labelledby="staff-modal-title" onMouseDown={(event) => event.stopPropagation()} onKeyDown={(event) => trapTabKey(event, staffModalRef.current)}><div className="modal-head"><div><span>DEPARTMENT PROFILE</span><h2 id="staff-modal-title">부서별 정원·현원 설정</h2></div><button className="close-button" onClick={() => setShowStaffModal(false)} aria-label="닫기"><X size={19} /></button></div><div className="modal-fields" style={{display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "16px"}}>{DEPARTMENTS.map((dept) => (<div key={dept} style={{padding: "12px", border: "1px solid var(--border)", borderRadius: "6px"}}><h3 style={{margin: "0 0 12px 0", fontSize: "12px", fontWeight: "600"}}>{dept}</h3><label style={{display: "flex", flexDirection: "column", gap: "4px", marginBottom: "8px"}}>정원<input value={staffData[dept]?.capacity || ""} onChange={(event) => setStaffData({...staffData, [dept]: {...(staffData[dept] || {}), capacity: event.target.value}})} inputMode="numeric" style={{padding: "6px", borderRadius: "4px"}} />명</label><label style={{display: "flex", flexDirection: "column", gap: "4px"}}>현원<input value={staffData[dept]?.current || ""} onChange={(event) => setStaffData({...staffData, [dept]: {...(staffData[dept] || {}), current: event.target.value}})} inputMode="numeric" style={{padding: "6px", borderRadius: "4px"}} />명</label></div>))}</div><div className="modal-actions"><AppButton variant="ghost" onClick={() => setShowStaffModal(false)}>취소</AppButton><AppButton variant="primary" onClick={saveStaff}>저장</AppButton></div></div></div>}
